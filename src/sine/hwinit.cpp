...
void nvic_setup(void)
{
   ...
   nvic_set_priority(PWM_TIMER_IRQ, 2 << 4);

   nvic_enable_irq(NVIC_TIM1_BRK_IRQ);
   nvic_set_priority(NVIC_TIM1_BRK_IRQ, 0);  //Set highest priority

   nvic_enable_irq(NVIC_EXTI0_IRQ);
   nvic_set_priority(NVIC_TIM1_BRK_IRQ, 1 << 4); //higher priority than PWM
}

/**
* Setup timer for generating over current
